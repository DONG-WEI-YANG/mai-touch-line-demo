...
async function main() {
  ...
  const groups=[
    ...
    [serviceActions('報修與服務','點選下方按鈕開始操作。',[{type:'postback',label:'我要報修',data:'flow=repair.report'}]),serviceActions('我的行事曆','查看預約歷史與服務紀錄。',[{type:'uri',label:'開啟行事曆與紀錄',uri:'https://mai-touch-history-20260908.web.app'}])],
  ];
