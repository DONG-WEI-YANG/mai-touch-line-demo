...
it('uses postback navigation and SVG-derived HTTPS icons',()=>{
  ...
  expect(home.contents.body.contents).toHaveLength(6);
  expect(home.contents.body.contents.every(c=>c.action.type==='postback')).toBe(true);
  expect(JSON.stringify(facilityCarousel('zh-TW'))).toContain('/line-icons/pool.png');
  expect(serviceHome('housekeeper','zh-TW').contents.body.contents.some(c=>c.action.data==='nav=workorders')).toBe(true);
});
