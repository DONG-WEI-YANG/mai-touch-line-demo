...
最新發布補充：UX-08、UX-09、UX-11已部署GCP record-ux-20260910；下文「本機未發布」為修正當時紀錄。753項測試、型別、lint通過，16組LINE訊息與rich menu官方格式通過，公開health與官方webhook/test成功200。資料庫完整性／外鍵通過，28表筆數未減少；真實手機點擊仍待驗收。
...
- 狀態：本機已區分清單與單號詳情；詳情不再提供自身查詢按鈕，無關聯顯示尚無關聯紀錄，提供回紀錄清單／首頁。文字與postback查單均傳遞查詢情境；尚未發布或實機驗收。
...
- 狀態：本機已移除新工單卡的轉派按鈕，保留接單／拒絕；舊卡轉派改回覆尚未變更指派，提供管理後台與首頁入口。尚未發布或實機驗收。
...
- 狀態：本機已改為Flex預約清單／Web管理入口，取消說明不再導向工單；明確要求在我的預約確認取消並查看結果。尚未發布或實機驗收。
