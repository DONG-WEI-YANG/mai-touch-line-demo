...
import { createSqliteBooking } from './sqliteBooking';

type BookingInput = {
  userId: number; amenityId: number; date: string;
  startTime: string; endTime: string; guestCount: number; notes?: string; requestId?: string;
};

/** SQLite writers use an IMMEDIATE transaction; SQL triggers also guard other writers. */
export async function createCheckedBooking(input: BookingInput) {
  await db.getAmenityById(input.amenityId);
  if (dbManager.getType() === "sqlite") return createSqliteBooking(dbManager.getRawSqlite(), input);
  return runExclusive(`booking:${input.amenityId}:${input.date}`, async () => {
    ...
    try { validateBookingWindow(input, amenity); }
    catch (error) { throw new TRPCError({code:'BAD_REQUEST',message:error instanceof Error?error.message:'預約時段無效'}); }
